...
The _Multi Slicing_ Requirements Class extends the subsetting Requirements Class with the ability to request multiple slices at specific coordinates, including for <<term-irregular-grid, irregular grids>> and <<term-variable-type,categorical>> values.
...
The _Field Selection_ (Range Subsetting) Requirements Class defines parameters for selecting a subset of the bands (defined in the <<term-range-type, Range Type>> which is described in the schema resource of the collection) to retrieve from Range Sets.
