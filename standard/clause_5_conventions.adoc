...
=== Identifiers
The normative provisions in this standard are denoted by the URI `https://www.opengis.net/spec/ogcapi-coverages-1/1.0`.

All requirements and conformance tests that appear in this document are denoted by partial URIs which are relative to this base.

=== Examples

Most of the examples in this Standard are expressed as JSON. JSON was chosen because it is widely understood by implementers and easy to include in a text document. This convention should NOT be interpreted as a requirement that JSON must be used. Implementors are free to use any format they desire as long as there is a Conformance Class for that format and the API implementation advertises its support for that Conformance Class.

=== Schema

JSON Schema (version 2020-12) is used throughout this Standard to define the structure of resources. These schema are typically represented using YAML encoding. This convention is for the ease of the user. It does not prohibit the use of another schema language or encoding. Nor does it indicate that JSON schema is required. Implementations should use a schema language and encoding appropriate for the format of the resource.
...
Note that the CURIE (Compact URI) based on the pattern `[ogc-rel:<relation>]` is also considered equivalent.
...
* **item**: The target IRI points to a resource that is a member of the collection represented by the context (used by the templated link pointing to individual coverage tiles in a coverage tileset. Not used in the document representing the list of collections because the context is an item of the array of collection in that case, so `self` is used instead).
...
* **http://www.opengis.net/def/rel/ogc/1.0/tiling-schemes**: The target IRI points to a resource that lists one or more TileMatrixSets according to the OGC 2D-TMS standard.

The following link relation types specified in the _OGC API - Features - Part 5 / OGC API - Common - Part 3: Schemas_ Standard are used:
...
* **http://www.opengis.net/def/rel/ogc/1.0/data-meta**: The target IRI points to a resource representing general metadata for the context resource (e.g. collection of geospatial data) (e.g., <<ISO_19115-1,ISO-19115>> -- not the domain/application metadata as defined in CIS).

Each resource representation includes an array of links. Implementations are free to add additional links for all resources provided by that specific Web API instance,
including but not limited to those defined in the Coverages API requirements classes.

[[conventions-interpolation-methods]]
=== Interpolation methods

Recommendations for supporting a `Prefer:` request header in the <<rc_scaling_spatial, Scaling>> and <<rc_sparse-dimensions-collapse-spatial, Collapsing Sparse Dimensions>> requirements classes allow clients to specify a preferred
<<term-interpolation-method,interpolation method>> using a URI (or CURIE) referencing interpolation methods registered on the OGC definition server at
https://www.opengis.net/def/interpolation. The interpolation methods referenced in this document are:

* **https://www.opengis.net/def/interpolation/OGC/1/none**: Only return a value if an exact data point is available, return NODATA otherwise (hit or miss)

* **https://www.opengis.net/def/interpolation/OGC/1/linear**: Perform a linear interpolation (weighted average) between the values being represented,

* **https://www.opengis.net/def/interpolation/OGC/1/nearest-neighbor**: Use the nearest data point,

* **https://www.opengis.net/def/interpolation/OGC/1/quadratic**: Perform a quadratic interpolation among data points around the slicing point along the dimension,

* **https://www.opengis.net/def/interpolation/OGC/1/cubic**: Perform a cubic interpolation among data points around the slicing point along the dimension,

* **https://www.opengis.net/def/interpolation/OGC/1/mean**: Use the mean of all values being represented,

* **https://www.opengis.net/def/interpolation/OGC/1/minimum**: Use the minimum of all values being represented,

* **https://www.opengis.net/def/interpolation/OGC/1/maximum**: Use the maximum of all values being represented,

* **https://www.opengis.net/def/interpolation/OGC/1/median**: Use the median of all values being represented,

* **https://www.opengis.net/def/interpolation/OGC/1/modal**: Use the most frequent value of all values represented,

* **https://www.opengis.net/def/interpolation/OGC/1/next**: Use the next available data point,

* **https://www.opengis.net/def/interpolation/OGC/1/previous**: Use the previous available data point,

where, as an example, the equivalent CURIE form for linear interpolation is `[ogc-interpolation:linear]`.

This list is not exhaustive -- additional interpolation methods may be proposed for addition to the https://www.opengis.net/def[OGC Definition Server],
and different URIs may be understood by specific implementations when an adequate URI has not yet been registered for other
...
Everywhere a URI is expected in a query parameter, an implementation also needs to support a safe CURIE.
