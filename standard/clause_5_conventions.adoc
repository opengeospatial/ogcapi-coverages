...
The following link relation types specified in the _OGC API - Tiles_ candidate standard are used:
...
* **http://www.opengis.net/def/rel/ogc/1.0/coverage-rangeset**: The target IRI points to a resource representing the coverage's range set component (the values for the range).
* **http://www.opengis.net/def/rel/ogc/1.0/coverage-rangetype**: The target IRI points to a resource representing the coverage's range type component (the list of fields and their description).
* **http://www.opengis.net/def/rel/ogc/1.0/coverage-domainset**: The target IRI points to a resource representing the coverage's domain set component (the list of dimensions/axes and their bounds or discrete values)
* **http://www.opengis.net/def/rel/ogc/1.0/coverage-metadata**: The target IRI points to a resource representing the coverage's metadata component (usually domain/application specific -- not general geospatial metadata such as ISO-19115, which should instead use `http://www.opengis.net/def/rel/ogc/1.0/data-meta`).
