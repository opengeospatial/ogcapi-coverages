...
For OGC API - Coverages, this native CRS corresponds to the default output CRS (which may be overridden with a `crs` query parameter if the implementation supports the "Coordinate Reference System" requirements class).
...
spans an area of interest, and is unrelated to how the data might be stored internally or retrieved most efficiently.
