...
This operation is defined in the `Core` conformance class of API-Common. No modifications are needed to support `Coverage` resources. The `Core` conformance class specifies only one way of performing this operation:

. Follow the `{root}/` path

Support for the `{root}/` path is required by API-Common.
...
. Follow the `/collections/{coverageId}` path
. Follow the `dataset` link on the collections info page

Support for both the `/collections/{coverageId}` path and the `dataset` link is required by API-Common.

The {coverageId} parameter is the unique identifier for a single coverage on the API.
...
The paths discussed in this section are all branches off of the `/collections/{coverageId}/coverage` root.

[[coverage-clause]]
==== Coverage Offering

Returns a general coverage offering description consisting of envelope, rangetype, and service metadata such as the coverage's native format
