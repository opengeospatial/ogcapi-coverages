...
. Issue a `GET` request on the `{root}/collections/{coverageId}` path
...
This section is about coverage access, which is driven by the coverage structure. That structure contains four principle components:
