...
|<<api-definition,{root}/api>> |API Description
...
|{root}/collections/{collectionid}/coverage/all |returns all of the above namely the coverage's domainset, rangetype, metadata, and rangeset comparable to a GetCoverage response
|===
...
include::requirements/core/REQ_core_landing_page.adoc[]
...
Every API is expected to provide a definition that describes the capabilities of that API. This definitionb can be used by developers to understand the API, by software clients to connect to the server, or by development tools to support the implementation of servers and clients.
...
The Response to the Conformance operation is a list URLs. Each URL identifies an OGC Conformance Class for which this API claims conformance. The schema for this resource is defined in API-Common and provided for reference in <<conformance-schema>>.

[#conformance-schema,reftext='Conformance Response Schema']
.Conformance Response Schema
[source,yaml]
----
include::common/schemas/confClasses.yaml[]
----
...
Support for both the `/collections` path and the `dataset` link is required by API-Common.
...
[source,yaml]
----
include::common/schemas/collections.yaml[]
----
...
[source,yaml]
----
include::common/schemas/collectionInfo.yaml[]
----
