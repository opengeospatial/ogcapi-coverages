[[encodings-section]]
== Requirements Classes for Representations

The Coverages API Standard does not mandate any particular encoding or format in which to return the `../coverage` resource.
Coverages can be encoded in any suitable data format.
However, it does define requirements class for representations which are expected to be commonly supported in implementations of this standard.
...
None of these formats is mandatory. An implementor of the Coverages API Standard may choose to implement none of them, selecting different formats/encodings instead.

The media type used to format/encode a response to a request shall be determined based on HTTP content negotiation, as specified in _OGC API - Common_.
If not using content negotiation, the format/encoding must follow the media type described in the link to the resource from the collection.
...
The HTML requirements class defines at a high level the ability to retrieve an HTML response for the coverage resource intended primarily for users accessing a Coverages API endpoint from a Web browser.
...
The netCDF requirements class defines support for encoding a coverage response according to the https://docs.ogc.org/is/10-090r3/10-090r3.pdf[OGC netCDF standard].
...
The CIS JSON requirements class defines support for encoding a coverage response according to the http://www.opengis.net/doc/IS/cis/1.1.1[OGC Coverage Implementation Schema (CIS) 1.1] or a later version,
with the version negotiated via the `version` parameter of the `application/cis+json` media type.

The http://www.opengis.net/doc/IS/cis/1.1.1[Coverage Implementation Schema (CIS)] defines a logical model and physical encodings (e.g., CIS JSON) which can be used to retrieve the coverage,
...
The CIS JSON structures are defined using JSON Schema (https://schemas.opengis.net/cis/1.1/json/coverage-schema.json for version 1.1).
A draft corrigendum for version 1.1.1 is available at https://github.com/opengeospatial/coverage-implementation-schema/blob/main/standard/schemas/1.1/json/coverage-schema.json, with updated examples
available at https://github.com/opengeospatial/coverage-implementation-schema/tree/main/standard/schemas/1.1/json/examples .

The media type `application/cis+json` is planned to be registered for CIS JSON.
...
This Coverages API CIS JSON requirements class includes a recommendation for implementations to support a `components` media type parameter which can be used by a client to retrieve only some of these aspects of the coverage e.g.,
`Accept: application/cis+json; components="domainset,rangetype"`. Information equivalent to the CIS _DomainSet_ is already included in the collection description resource `extent` property,
...
This capability also allows clients to retrieve a subset of the domainset when using `Accept: application/cis+json; components="domainset"` together with
one or more subsetting parameters (`subset`, `bbox` or `datetime`).

The CIS standard defines a number of alternative methods in which a coverage encoding can be structured.
...
_GeneralGridCoverage_ (for coverages consisting of one or more dimensions of <<term-regular-grid, regular>> and/or <<term-irregular-grid, irregular grids>>) or a _DirectMultiPoint_ coverage (for point clouds), excluding
...
Implementations of this requirements class wishing to present different constituent parts of a coverage as separate resources can consider implementing an eventual _Scenes_ extension, where
...
The LAS requirements class defines support for encoding a coverage response according to the https://docs.ogc.org/cs/17-030r1/17-030r1.pdf[OGC LAS Community Standard].
...
https://docs.ogc.org/cs/24-070r1/24-070r1.pdf[OGC LAZ Community Standard], adding compression to the https://docs.ogc.org/cs/17-030r1/17-030r1.pdf[OGC LAS Community Standard].
...
Since PNG encoding is limited to an integer data values, this PNG reponse format requirements class defines additional parameters and response headers allowing a client to request
a specific scale factor and offset to be used to quantize coverage values, and allowing an implementation to inform the client of the scale factor and offset
...
The (Geo)Zarr requirements class defines support for encoding a coverage response according to the https://docs.ogc.org/cs/21-050r1/21-050r1.pdf[OGC Zarr Community Standard], within a https://www.iso.org/standard/60101.html[Zip] archive, with
