...
^|A |If a particular collection such as point cloud is suitable for a LAS representation, the coverage resource as defined in the Coverages API Core requirements class SHALL support negotiating a https://portal.ogc.org/files/?artifact_id=74523[LAS] response using the `application/vnd.las` media type.
