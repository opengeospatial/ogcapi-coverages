...
^|B |With the exception of considerations for the requirements described in <<rc_sparse-dimensions-collapse-temporal>>,
only that part of the coverage that falls within the bounds of the subset expression SHALL be returned.
^|C |With the exception of considerations for the requirements described in <<rc_sparse-dimensions-collapse-temporal>>, if a single point is provided for an axis, the coverage response SHALL be the result of a slicing operation for the dimension corresponding to that axis, reducing the dimension.
...
^|F |A 400 error status code SHALL be returned for an axis name other than `time`, an alias for `time` (see <<spatial-aliases-recommendation, recommendation>>), or any other axis supported in the context of other requirements classes (such as spatial and general subsetting).
...
^|H |For the `time` axis, coordinates SHALL be interpreted as values for the named axis of the `trs` specified in the collection's temporal extent, or Gregorian UTC time
(as specified in link:https://tools.ietf.org/html/rfc3339[RFC 3339],
extended with support for an hour value of 24) if it is not specified in the temporal extent.
Interval ends SHALL be evaluated using implicit duration thickness based on the smallest specified component, with the exception of an extended hour value of `24` (valid only with zero sub-components under ISO 8601 rules) which SHALL be interpreted exactly as hour `00` of the following calendar day, evaluating up to, but excluding, that temporal instant without applying implicit thickness.
