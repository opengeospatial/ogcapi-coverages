...
Because the time dimension is sparse due to some satellites only covering some of the Earth's surface on a given day, data is not available for that specific date for some of the
...
definition server's (https://www.opengis.net/def/interpolation) for specific dimensions being sliced. Example: `Prefer: interpolation=time([ogc-interpolation:linear])`
...
which will override interpolation methods specified in any `interpolation` preference for any dimension, using a safe CURIEs or URI from the OGC
definition server's (https://www.opengis.net/def/interpolation). Example: `Prefer: properties-interpolation=landCover([ogc-interpolation:nearest-neighbor])`.
