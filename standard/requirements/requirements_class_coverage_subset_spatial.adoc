...
===== Examples of applying the permission for slicing on sparse dimensions
...
In a different scenario, the slicing dimension could be space while the trim is performed on the temporal dimension:
...
[[rec_slice_spatial_prefer]]
[width="90%",cols="2,6a"]
|===
^|*Recommendation {counter:per-id}* |*/rec/subsetting-spatial/slice-prefer-header*
^|A |When supporting slicing on sparse dimensions, the Implementation SHOULD understand a `Prefer:` header (as defined in https://datatracker.ietf.org/doc/html/rfc7240[RFC 7240]) value provided by the client hinting at the preferred interpolation method.
...
definition server's (https://www.opengis.net/def/interpolation) for specific dimensions being sliced. Example: `Prefer: interpolation=time([ogc-interpolation:nearest-neighbor])`
^|C |The implementation SHOULD support an `properties-interpolation=` value for that `Prefer:` header selecting one specific interpolation method using a safe CURIEs or URI from the OGC
...
^a|D | The implementation SHOULD consider support the following interpolations methods, with the following meaning in the context of the slicing sparsed dimensions permission:
...
^|E | The implementation SHOULD use either `[ogc-interpolation:previous]` or `[ogc-interpolation:nearest-neighbor]` when no preference is specified by the client.
^|F | The implementation SHOULD use a response header `Preference-Applied:` when it applied the preference specified by the client, for example,
...
^|G | The implementation SHOULD support the `Prefer: handling` parameter where a value of `handling=lenient` would simply ignore an unsupported preference, while a
