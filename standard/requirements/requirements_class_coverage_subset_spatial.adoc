...
IMPORTANT: Slicing (a `subset` specifiying a single value for a particular dimension) is mutually exclusive with scaling on that same dimension
(using the `resolution` query parameter as defined in the relevant Scaling requirements class).
