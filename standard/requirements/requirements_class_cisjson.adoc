...
^|A |If a particular collection is suitable for a CIS JSON representation, the coverage resource as defined in the Coverages API Core requirements class SHALL support negotiating a CIS JSON response using the `application/json` media type.
...
^|C |The coverage object response SHALL be of type _CoverageByDomainAndRange_ (not _CoverageByPartitioning_, an implementation can instead consider supporting an eventual "Scenes" extension to present individual parts of a coverage).
...
^|A |The coverage request operation SHOULD support a `profile` query parameter enabling the selection of one or more specific CIS coverage components to include (domainset, rangetype, rangeset and/or metadata).
...
^|D |An implementation SHOULD return the metadata when explicitly requested to do so with the `profile` query parameter.
|===
...
^|A |An implementation MAY omit the CIS domain-specific metadata by default as part of the coverage response.
|===
