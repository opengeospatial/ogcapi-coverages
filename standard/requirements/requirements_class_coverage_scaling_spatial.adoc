...
[[rec_coverage_scaling-spatial_prefer]]
[width="90%",cols="2,6a"]
|===
^|*Recommendation {counter:rec-id}* |*/rec/scaling-spatial/scale-prefer-header*
^|A |When supporting scaling on spatial dimensions, the Implementation SHOULD understand a `Prefer:` header (as defined in https://datatracker.ietf.org/doc/html/rfc7240[RFC 7240]) value provided by the client hinting at the preferred interpolation method.
...
[[per_coverage_scaling-spatial-permission-specific-resolutions]]
[width="90%",cols="2,6a"]
|===
^|*Permission {counter:per-id}* |*/per/coverage-scaling-spatial/specific*
...
^|C |When neither `availableResolutions` nor `minCellSize` is present, implementations MAY return a 4xx error for a requested resolution more detailed than the `resolution` declared for the dimension.
^|D |Implementations MAY return either the closest resolution available, or a 4xx client error when requesting a resolution differing by more than 5% from any listed resolution.
|===
