...
definition server's (https://www.opengis.net/def/interpolation) for specific dimensions being scaled. Example: `Prefer: interpolation=lat([ogc-interpolation:nearest-neighbor]),lon([ogc-interpolation:nearest-neighbor])`
...
NOTE: A common use case for the Scaling requirements classes is the ability for clients to request downsampled (coarser)
