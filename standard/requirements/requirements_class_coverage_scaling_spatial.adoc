...
[width="90%",cols="2,6a"]
|===
^|*Recommendation {counter:per-id}* |*/rec/scaling-spatial/scale-prefer-header*
...
^a|D | The implementation SHOULD consider support the following interpolations methods, with the following meaning in the context of the dimension being scaled:
...
^|E | The implementation SHOULD use either `[ogc-interpolation:linear]` or `[ogc-interpolation:quadratic]` when no preference is specified by the client for fields with a variable
...
Clients wishing to upsample data can request the native resolution and perform upsampling on their, saving both bandwidth and server processing resources.

Example of specific spatial resolutions available:
