...
^|C |With the exception of considerations for the requirements described in <<rc_sparse-dimensions-collapse-general>>, if a single point is provided for an axis, the coverage response SHALL be the result of a slicing operation for the dimension corresponding to that axis, reducing the dimension.
