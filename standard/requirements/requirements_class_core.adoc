...
For both <<term-regular-grid, regular>> and <<term-irregular-grid, irregular grids>>, a `cellsCount` property specifies the number of cells.
For <<term-regular-grid, regular grids>>, a `firstCoordinate` and `resolution` is specified, with optional `relativeBounds`.
For <<term-irregular-grid, irregular grids>>, a list of `coordinates` is specified, with optional `boundsCoordinates` pairs.
The collection description will also include a link to the logical schema describing the coverage fields available as JSON Schema using the link relation type https://www.opengis.net/def/rel/ogc/1.0/schema.
