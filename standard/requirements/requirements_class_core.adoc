...
^|B |If some conformance classes of this Standard are only supported by particular collections, Implementations SHALL
the API instance has a mechanism to advertise conformance classes, each collection supporting additional
requirements classes SHALL be listed in a resource linked to from the collection description as described in
