...
in particular the `/collections/{collectionId}/coverage` which is likely to return a large amount of data.
A `HEAD` response can help clients validating the georeferencing of the content (see <<req_core_response-headers, response headers for spatiotemporal referencing>>),
the interpolation methods used (see <<rec_core_interpolation-applied, headers for interpolation methods>>), and avoid wasteful exchange of data that
