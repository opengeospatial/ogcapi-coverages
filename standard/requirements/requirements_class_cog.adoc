...
^|B |Implementations SHALL only be expected to support HTTP range requests on the coverage resource when used with no query parameters, or with the <<req_core-native-resolution,native resolution>> (empty values for all dimensions).
|===
