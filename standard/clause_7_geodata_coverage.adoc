...
This standard describes use of the API-Common `Collections` requirements class to support collections of coverages.
...
The `Collections` operation is defined by the http://www.opengis.net/spec/ogcapi-common-2/1.0/req/collections Requirements Class. The requirements applicable to the `Collections` operation are listed in <<collections-requirements-table>>. 
...
(Note: Requirement identifiers are relative to http://www.opengis.net/spec/ogcapi-common-2/1.0/)

Optional query parameters for use with the `Collections` operation are defined by the http://www.opengis.net/spec/ogcapi-common-2/1.0/req/simple-query Requirements Class.
...
The resources definitions and operations required to access a Collection Resource are defined in the http://www.opengis.net/spec/ogcapi-common-2/1.0/req/collections Requirements Class of API Comon Part 2. The requirements applicable to the `/Collections/{collectionId}` operation are listed in <<collection-requirements-table>>. 

[#collection-requirements-table,reftext='{table-caption} {counter:table-num}']
.Collection Resource Requirements 
[width="90%",cols="2,6a",options="header"]
|====
^|**Section** ^|**Requirements** 
^|Operation |/req/collections/src-md-op
^|Response |/req/collections/src-md-success
^|Content |/req/collections/collection-definition, +
...
Due to the multi-dimensional nature of coverages, this `GeoData Coverage` Requirements Class extends the Collection Resource defined in API Common by replacing the `Extent` module with the `Extent-UAD` module. The `Extent-UAD` module defines an optional extension of the `Extent` module. It has been designed so that a Collection with a Extend-UAD will still comply with API-Common Part 2. Likewise, a Collection which is conformant with API-Common Part 2 will also be conformant with API-Coverages.

include::../api_modules/extent-uad/requirements/REQ_extent-uad-definition.adoc[]
...
OGC API Common only supports access to collection metadata. OGC API - Coverages defines the operations and schemas specific to Coverages.
