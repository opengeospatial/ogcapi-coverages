...
The  is being rechartered Web Coverage Service (WCS) Standards Working Group (SWG). 
...
* Develop and maintain standards for metadata exposed through Coverage Web Services or APIs.
* Develop and maintain standards for the encoding of coverage resources exposed through Coverage Web Services or APIs.
...
Much of the coverage data produced by EO satellite programs is used by scientists to help understand environmental change and how communities can improve their management of natural resources. Other applications include, for example, competitor analysis in business and geospatial intelligence in defense and security. With the volume of coverage data growing daily, there is an significant need to find ways to leverage recent advances in technology that can improve the ability of users to discover and access coverages. The goal of the OGC API - Coverages standards work is to leverage some of those technological advances, in particular Web APIs.

=== Scope of Work

The Web Coverage Service (WCS) is a suite of Web Service standards which have been successfull in delivering interopeable coverage resources to the Geospatial community. WCS has proven value and its's maintenance and extension will be continued under this SWG.

Web APIs are a newer technology which is displacing Web Services in many communities. The OGC API-Coverages suite of standards will provide WCS-like capablities through OGC Web API Standards. 

The OGC API-Coverages concept has been demonstrated in various OGC Innovation Program initiatives.

The {swgname} SWG will build on those preliminary efforts to more fully develop and document an OGC API-Coverages-Part 1:Core candidate standard that will provide a modernized, common, and consistent Web API interface to Coverage resources on the the Web. This API will align with the current architecture of the Web and the Spatial Data on the Web Best Practices (https://www.w3.org/TR/sdw-bp/).

Following the example of WCS, the {swgname} will develop additional Web API standards which add optional capabilties to the API-Coverages Core. The result will be a suite of modular standards which can be assembled to meet the specific needs of a system or product developer. 

* Architecture: The standards developed by the {swgname} SWG will align with prior OGC work for Coverages, Web Services, and Web APIs. In addition, they will align with industry common practices in Web Services or Web APIs as appropriate.  

* Metadata Encodings: The first version of the API-Coverages Standard will support JSON and HTML as encodings for descriptions of Coverage resources in the API. No encoding will be mandatory and other encodings may be used as well.

* Coverage Encodings: The first version of the API-Coverages Standard will support XML (ISO 19123-2) and JSON/GeoJSON encodings. No encoding will be mandatory and other encodings may be used as well.

NOTE: For the following section, the information model should be ISO 19125 for Coverages with OpenAPI and Web Service Architecture for "services".
...
In addition, this SWG continues work on the WCS.SWG Core and Extensions, including both maintenance and new specification development.
...
The starting point for the work will be the draft document that is currently on the proposed SWG's repository ({githubrepo}). This charter recognizes the prior work done by the {base_swg}. Upon approval of this Charter, responsibility for {swgname} will be transferred to the proposed {swgname} SWG.
...
*	A final version of the "OGC-API - Part 1: Core" document for submission to the TC.
