...
:githubrepo: https://github.com/opengeospatial/ogcapi-coverages
:base_swg: Web Coverage Service (WCS) SWG
:target_start_date: July 2020
:target_end_date: March 2027
...
The OGC members listed below have proposed that the Web Coverage Service SWG be rechartered as the "{swgname} SWG".
...
* You must notify the SWG chair of your intent to participate to the meetings. Members may do so by logging onto the OGC Portal and navigating
to the Observer page and clicking on the link for the SWG they wish to join and;
...
* Develop and maintain the OGC Web Coverage Processing Service standard.
...
The quarterly Space Foundation https://promo.spacefoundation.org/tsr/quarterly-reports/TheSpaceReport19Q1.pdf[2019 first quarter Space Report]
states that 38 percent of the 465 spacecraft deployed in 2018 were deployed for Earth Observation or Remote Sensing missions.
A majority of those spacecraft were deployed for commercial purposes. In a separate http://www.euroconsult-ec.com/earthobservation[report] by a consulting firm specializing in
space markets , the market for data and services derived from Earth observation (EO) satellites is forecasted to reach $8.5 billion by 2026.

Much of the coverage data produced by EO satellite programs is used by scientists to help understand environmental change and how
communities can improve their management of natural resources. Other applications include, for example, competitor analysis in
business and geospatial intelligence in defense and security.

In the European INSPIRE legal framework for a common Spatial Data Infrastructure coverages play a central role.
In 9 of the 13 themes data appear modeled as coverages, such as Elevation, Land cover, Orthoimagery, Soil, Energy resources,
Species distribution, Natural risk zones, and Geology.

With the volume of coverage data growing daily, there is an significant need to find ways to leverage recent advances
in technology that can improve the ability of users to discover and access coverages. The goal of the OGC API - Coverages
Standards work is to leverage some of those technological advances.
=== Scope of Work
...
* Web Coverage Processing Service (WCPS) is a high-level, declarative query language for spatio-temporal geo datacubes allowing to extract, process, analyze, and combine grid coverages.
* OGC API - Coverages is a service model based on OpenAPI frameworks. These have helped make describing and sharing API definitions more suitable for interoperability standardization. The concept of a OGC API - Coverages was demonstrated in various OGC Innovation Program initiatives.

Regarding OGC API - Coverages, the {swgname} SWG will build on those preliminary efforts to more fully develop and document an
OGC API - Coverages candidate standard that will provide a modernized, common, and consistent interface to services that aligns
with the current architecture of the Web and the Spatial Data on the Web Best Practices (https://www.w3.org/TR/sdw-bp/).
...
Part 1 will cover basic capabilities to retrieve coverage data, including capability for retrieving a subset of the domain and/or range, resample (scale) the data,
and request output in a particular CRS as optional requirements classes.
Capabilities for richer {resource} interfaces or extension for unique geospatial resource considerations will be specified in additional parts.

Formal approval of the core OGC API - Coverages is envisaged to take place nearer October 2026.

The {swgname} SWG is developing Coverage encodings, in addition to the existing CIS XML and JSON encodings, for:

* CoverageJSON

The {swgname} SWG provides maintenance and enhancement on OGC the WCS and WCPS standards.
...
The {swgname} SWG work program also include development of extensions to the OGC API - Coverages Core Standards providing additional capabilities:

* OGC API - Coverages - Part 2: Filtering, Deriving and Aggregating Fields
* OGC API - Coverages - Part 3: Scenes

Part 2 will address a number of server-side processing use cases (e.g., calculation of an NDVI index) by leveraging the OGC Common Query Language (CQL2).
Part 3 will address the transactional aspect by allowing to manage (Create/Retrieve/Replace/Update/Delete) individual scenes (or parts / granules) of a coverage,
listing and discovering these scenes, while also supporting STAC items representations for these scenes.

The {swgname} SWG will define requirements classes as part of OGC API - Coverages for coverage data responses in format standards:

* GeoTIFF (including Cloud Optimized GeoTIFF)
* NetCDF (including version 4 based on HDF5)
* CIS JSON
* CoverageJSON
* LAS and LASZip
* PNG
* JPEG XL
* JPEG 2000 (including GMLJP2 georeferencing)
* (Geo)Zarr

However, OGC API - Coverages remains agnostic of encoding and any additional supported encodings can be negotiated between client and server.
...
* ISO 19123-1:2023 Coverage Fundamentals (identical to OGC Abstract Topic 6.1)
* ISO 19123-2:2026 Coverage Implementation Schema (under re-adoption by OGC)
* ISO 19123-2:2023 Coverage Processing Fundamentals (identical to OGC Abstract Topic 6.3, based on OGC WCPS)
* OGC Web Coverage Service (WCS)
* OGC Web Coverage Processing Service (WCPS) geo datacube query language
* EO-GeoJSON
* GML in JPEG2000
* GeoTIFF
* HDF5
...
|Peter Baumann | Constructor (ex Jacobs) University, rasdaman GmbH
